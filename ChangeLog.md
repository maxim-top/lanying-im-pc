...
1.群组增加全员禁言UI界面展示功能。
2.单聊和群聊增加批量转发消息功能。
3.单聊和群聊增加转发消息批量转发给多个联系人功能。
4.增加解析appConfig中批量转发处理参数的逻辑。
5.修改新增批量转发后群聊界面的CSS样式，确保群聊消息列表样式展示正常。
6.修改开发依赖项目，确保markdown代码消息能政策展示。

### v3.2.5 20240828
1.紧急修复文本字段无法复制粘贴到消息输入框的问题。

### v3.2.4 20240828
1.PC 端新增拖拽和复制发送图片即文件消息的功能。
2.Windows 安装包和 MAC 安装包增加了安装包签名处理。
3.修复 Windows 环境下窗口左上角只有文件名称没有小图标的问题。
4.修复 Windows 安装包没有显示开发者的问题。
5.更新 README 文档内关于签名处理的实现及取消签名处理应该怎样操作的文档。
6.PC 端新增下载接收到的文件及图片的功能。
7.PC 端修改了系统托盘的实现样式及功能，MAC 端新增了系统托盘的支持。
